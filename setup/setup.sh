...
# Updated parser so rule isn't needed

if [ -f "/etc/udev/rules.d/99-custom-camera.rules" ]; then
    echo "Found /etc/udev/rules.d/99-custom-camera.rules - removing it..."
    sudo rm "/etc/udev/rules.d/99-custom-camera.rules"
    if [ $? -eq 0 ]; then
        echo "Successfully removed 99-custom-camera.rules"
        sudo udevadm control --reload-rules
	sudo udevadm trigger
    else
        echo "Error: Failed to remove 99-custom-camera.rules"
        exit 1
    fi
else
    echo "/etc/udev/rules.d/99-custom-camera.rules does not exist - nothing to remove"
fi
